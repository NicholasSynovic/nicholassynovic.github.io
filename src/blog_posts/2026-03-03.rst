...
.. note::
   **Edits To This Page**
   ...
   - **03-04-2026**: Added links to the Gemini CLI Free Tier API calling
     documentation and clarified the capabilities of connecting
     ``OpenCode`` to Google Gemini via OAuth
...
.. warning::
   By using the OAuth connector to Google Gemini, you are effectively connecting
   to the Gemini Code Assistance service via the Gemini CLI authentication flow [2]_.
   For more information regarding usage limits for free and paying users, see `this
   Gemini CLI documentation page <https://geminicli.com/docs/resources/quota-and-pricing/#free-usage>`__.

To keep your configuration manageable, I recommend using either OAuth or an API
key, but not both. If you need to reset your credentials, use
``opencode auth logout`` to clear existing provider tokens.

In my testing for research-heavy workloads, Gemini 2.5 Flash via OAuth has
proven to be the most stable and responsive model within ``OpenCode``.
...
.. [1] https://opencode.ai
.. [2] https://github.com/jenslys/opencode-gemini-auth?tab=readme-ov-file#parity-notes
